...
class ReportService:
    def __init__(self) -> None:
        ...
        self.tracer = get_tracer("app.report_service")

    def _build_prompt(self, analysis_result: dict) -> str:
        with self.tracer.start_as_current_span("report.build_prompt") as span:
            span.set_attribute("has_predicted_class", analysis_result.get("predicted_class") is not None)
            span.set_attribute("evidence_count", len(analysis_result.get("evidence", [])))
            span.set_attribute("recommendation_count", len(analysis_result.get("recommendations", [])))
    ...
    def generate_report(self, analysis_result: dict) -> str:
        with self.tracer.start_as_current_span("report.generate") as span:
            predicted_class = analysis_result.get("predicted_class", None)
